...
def aggregate_returns(returns, convert_to):
    ...
    if convert_to == Periods.WEEKLY:
        grouping = [lambda x: x.year, lambda x: x.isocalendar()[1]]
    elif convert_to == Periods.MONTHLY:
        grouping = [lambda x: x.year, lambda x: x.month]
    elif convert_to == Periods.QUARTERLY:
        grouping = [lambda x: x.year, lambda x: int(math.ceil(x.month/3.))]
    elif convert_to == Periods.YEARLY:
        grouping = [lambda x: x.year]
    else:
        raise ValueError(
            f'convert_to must be {Periods.WEEKLY.value}, {Periods.MONTHLY.value} '
            f'or {Periods.YEARLY.value}'
        )
...
def annual_return(returns, period=Periods.DAILY, annualization=None):
    """
...
def cagr(returns, period=Periods.DAILY, annualization=None):
    """
...
def calmar_ratio(returns, period=Periods.DAILY, annualization=None):
    """
...
def capture(returns, factor_returns, period=Periods.DAILY):
