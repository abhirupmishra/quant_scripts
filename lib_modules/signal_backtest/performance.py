...
def factor_information_coefficient(
    ...
    def src_ic(group):
        ...
        f = group['factor']
        return group[utils.get_forward_returns_columns(factor_data.columns)] \
            .apply(lambda x: func(x, f)[0])
    ...
    return factor_data.groupby(grouper).apply(src_ic)
...
def factor_returns(factor_data,
    ...
    filter_criteria = utils.get_forward_returns_columns(factor_data.columns)
    weighted_returns = factor_data[filter_criteria].multiply(weights, axis=0)
